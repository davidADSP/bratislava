...
m_cols = fread(file = './desc_vars_no_txt.csv')[['x']]
...
new_m = matrix(0,nrow=1, ncol=length(m_cols))
i = 0
print(length(m_cols))
print(new_description)
for (term in m_cols){
  i = i + 1
  #if (term == 'easy access'){cat(term, ' ', grepl(term, tolower(new_description)))}
  #if (term == 'rear garden'){cat(term, ' ', grepl(term, tolower(new_description)))}
  if (grepl(term, tolower(new_description))){
    cat('adding ', term,'\n')
    new_m[,i] = 1
  }
}
cat('\n\n')
print(m_cols[which(new_m > 0)])
new_m = Matrix(new_m, sparse = TRUE)


out = cbind(Matrix(as.matrix(data_to_predict), sparse = TRUE), new_m)
#print(rowSums(new_m))
#print(out)
return (out)
}
